...
template<typename T, int dim>
MassSpringEnergy<T, dim>::MassSpringEnergy(const std::vector<T> &x, const std::vector<int> &e, const std::vector<T> &l2,
                                           const std::vector<T> &k) : pimpl_{std::make_unique<Impl>()} {
    pimpl_->N = x.size() / dim;
...
T MassSpringEnergy<T, dim>::val() {
    ...
    ParallelFor(256).apply(
                N, [device_val = device_val.viewer(), device_x = device_x.cviewer(), device_e = device_e.cviewer(),
                    device_l2 = device_l2.cviewer(), device_k = device_k.cviewer()] __device__(int i) mutable {
                    ...
                    device_val(i) = 0.5 * device_l2(i) * device_k(i) * (diff / device_l2(i) - 1) * (
                                        diff / device_l2(i) - 1);
                })
            .wait();
...
const DeviceBuffer<T> &MassSpringEnergy<T, dim>::grad() {
    ...
    device_grad.fill(0);
    ParallelFor(256).apply(
                N, [device_x = device_x.cviewer(), device_e = device_e.cviewer(), device_l2 = device_l2.cviewer(),
                    device_k = device_k.cviewer(), device_grad = device_grad.viewer()] __device__(int i) mutable {
                    int idx1 = device_e(2 * i); // First node index
                    int idx2 = device_e(2 * i + 1); // Second node index
                    T diff = 0;
                    T diffi[dim];
...
const DeviceTripletMatrix<T, 1> &MassSpringEnergy<T, dim>::hess() {
    ...
    ParallelFor(256).apply(
                N, [device_x = device_x.cviewer(), device_e = device_e.cviewer(), device_l2 = device_l2.cviewer(),
                    device_k = device_k.cviewer(), device_hess_val = device_hess_val.viewer(), device_hess_row_idx =
                    device_hess_row_idx.viewer(), device_hess_col_idx = device_hess_col_idx.viewer(), N] __device__(int
            i) mutable {
                    ...
                    Eigen::Matrix<T, dim, dim> H_diff =
                            scalar * (2 * diff_outer + (diff_vec.dot(diff_vec) - device_l2(i)) * Eigen::Matrix<T, dim,
                                          dim>::Identity());
